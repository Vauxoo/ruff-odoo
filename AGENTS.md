...
The guidance in this section applies to edits to the Vauxoo-specific `ODOO` rule group under `crates/ruff_linter/src/rules/odoo/`, or other work when it has been specifically mentioned by the user.
...
The `ODOO`/`OAPP` rules are published to <https://vauxoo.github.io/ruff-odoo/> from
...
### Rule names must match pylint-odoo

Every `ODOO`/`OAPP` rule keeps the exact name of the pylint-odoo or odoo-pre-commit-hooks
...
`pylint-disable-comment` (`ODOO047`) maps old pylint-odoo message codes and names onto Ruff
