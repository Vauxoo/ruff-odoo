# Vauxoo fork: adapted from `python/ruff/_find_ruff.py` (keep in sync); looks
# for the `ruff-odoo` binary next to the `ruff_odoo` module instead of `ruff`.
...
def find_ruff_bin() -> str:
    """Return the ruff-odoo binary path."""

    ruff_exe = "ruff-odoo" + sysconfig.get_config_var("EXE")
    ...
    targets = [
        ...
        (
            # On Windows, with module path `<prefix>/Lib/site-packages/ruff_odoo`
            _join(
                _matching_parents(_module_path(), "Lib/site-packages/ruff_odoo"),
                "Scripts",
            )
            if sys.platform == "win32"
            # On Unix,  with module path `<prefix>/lib/python3.13/site-packages/ruff_odoo`
            else _join(
                _matching_parents(
                    _module_path(), "lib/python*/site-packages/ruff_odoo"
                ),
                "bin",
            )
        ),
        # Adjacent to the package root, e.g., from `pip install --target`
        # with module path `<target>/ruff_odoo`
        _join(_matching_parents(_module_path(), "ruff_odoo"), "bin"),
    ...
    raise RuffNotFound(
        f"Could not find the ruff-odoo binary in any of the following locations:\n{locations}\n"
    )
