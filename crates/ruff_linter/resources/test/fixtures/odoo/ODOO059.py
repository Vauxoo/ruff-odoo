...
class Model:
    def method(self, var, name, count, total):
        ...
        _("Hello %s") % var  # ODOO059 (fixable)
        _("%s of %s") % (count, total)  # ODOO059 (fixable)
        _("Hello %(name)s") % {"name": name}  # ODOO059 (fixable)
        self.env._("Hello %s") % var  # ODOO059 (fixable)
        self.env._("Hello %s") % (var,)  # ODOO059 (fixable)
        _(name) % (count, total)  # ODOO059 (fixable: tuple converts for any term)
